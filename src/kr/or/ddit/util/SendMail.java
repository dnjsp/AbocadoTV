...
public class SendMail {
    ...
    public void SendTemporary(String mail,String tem) {
    	String BODY = String.join(
    			System.getProperty("line.separator"),
    			"<h2>Abocado TV에서 임시비밀번호를 보내드립니다.</h2><br>",
    			"<p>임시비밀번호는",
    			tem,
    			" 입니다.</p>"
    			);
    ...
    public void SendConsult(String category,String content, String checkinfo, String answer) {
    	String BODY = String.join(
    			System.getProperty("line.separator"),
    			"<h2>사용자가 상담메일을 보냈습니다.</h2><br>",
    			"<p>문의 내용은",
    			content,
    			" 이고,</p>",
    			"<p>답변 받는 방법은 < ",
    	...
    	try {        	
    		msg.setFrom(new InternetAddress(FROM, FROMNAME));
    		msg.setRecipient(Message.RecipientType.TO, new InternetAddress("whdhsp1@naver.com"));
    		msg.setSubject("상담메일");
    		msg.setContent(BODY, "text/html;charset=euc-kr");
    		Transport transport = session.getTransport();
    		transport.connect(HOST, SMTP_USERNAME, SMTP_PASSWORD);
